function [data, nsym, errors, SNR] = processQAM(waveform, diagnostics_on)
...
M = waveform.capture.modulation_order; % Equal to the number of QAM symbols.
block_length = waveform.capture.block_length; % number of symbols per marker frame.
symbol_rate = waveform.capture.symbol_rate; % symbol rate

signal = waveform.capture.signal;
sample_rate = waveform.capture.sample_rate; % Oscilloscope sample rate.
f_LO = waveform.capture.local_oscillator_frequency;

original_sample_frame = waveform.original.samples;
...
rxfilter = comm.RaisedCosineReceiveFilter;
    ...
    rxfilter.RolloffFactor = waveform.capture.rcf_rolloff;
...
symbolSync = comm.SymbolSynchronizer;
    ...
    symbolSync.NormalizedLoopBandwidth = 0.01;
...
start = waveform.capture.symbols_to_drop;
