function [scaled] = AWG_write(samples, markers, notes, filename, autoscale)
%% Documentation
...
% dynamic range while maintaining zero-mean.
...
if autoscale
    ...
    scaled = samples;
    % Old code (fuller dynamic range, but may produce undesired DC shift).
    %{
    [scale, idx] = max(abs(samples));
    if samples(idx) > 0 % If a positive value is largest
        samples = samples*(127/scale);
    else
        samples = samples*(128/scale);
        % can go to 128/scale, but produces offset from zero-mean when
        % working with signals that have + and - extremes of equal
        % magnitude.
    end
    %}
end
...
[fileID,errmsg] = fopen(filename, 'w', 'ieee-le');
if fileID < 0 
   error(errmsg);
end
